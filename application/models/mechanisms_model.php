...
class Mechanisms_model extends CI_Model
{
    ...
    public function check_availability_datim_id(){
        ...
        if($datim_id=$this->input->post('datim_id')){
    ...
    public function check_availability_kepms_id(){
        ...
        if($kepms_id=$this->input->post('kepms_id')){

            if ($query = $this->db->get_where('attribution_mechanisms', array('mechanism_id' => $kepms_id))){
                if(sizeof($query->result())==0){
                    return false;
                }
            }
